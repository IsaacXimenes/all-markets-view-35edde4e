...
export default function FinanceiroDespesasFixas() {
  ...
  const handleSelectAll = () => {
    if (selectedDespesas.length === despesasFixas.length) {
      setSelectedDespesas([]);
    } else {
      setSelectedDespesas(despesasFixas.map(d => d.id));
    }
  };
                ...
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[50px]">
                      <Checkbox 
                        checked={selectedDespesas.length === despesasFixas.length && despesasFixas.length > 0}
